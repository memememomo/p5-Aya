...
sub _prepare_from_dbh {
    ...
    if (! $builder ) {
        # XXX Hackish
        $builder = Aya::QueryBuilder->new(driver => $self->{driver_name} );
...
Aya is very simple DBI wrapper.
...
Aya classes are comprised of one distinct component:
...
This is the entry point to using Aya. You connect, insert, update, delete, select stuff using this object.


=head1 METHODS

Aya provides a number of methods to all your classes,
...
Aya does not support triggers (NOTE: do not confuse it with SQL triggers - we're talking about Perl level triggers). If you really want to hook into the various methods, use something like L<Moose>, L<Mouse>, and L<Class::Method::Modifiers>.

=head1 Row Object, Inflate/Deflate, Table Schema

Aya does not support Row Object, Inflate/Deflate, Table Schema. If you really want to use them, use L<Teng>.
...
=head1 REPOSITORY
  ...
  git clone https://github.com/memememomo/p5-Aya

=head1 LICENCE AND COPYRIGHT

Copyright (c) 2010, the Aya L</AUTHOR>. All rights reserved.
